...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      // Window only exists on the browser, so this acts as a double safety measure (good practice)
      ...
      if (lastVisit && now - parseInt(lastVisit) < twentyFourHours) {
        // If lastVisit exists i.e. not first time AND less than 24 hours
  ...
  const handleWelcomeComplete = () => {
    ...
    if (typeof window !== "undefined") {
      ...
      localStorage.setItem(
        "lastWelcomeTimestamp",
        new Date().getTime().toString()
      ); // Leave a memory in local storage that this page has been visited before
    }
  };
